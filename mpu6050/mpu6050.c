...
float Kalman_getAngle(Kalman_t *Kalman, float newAngle, float newRate, float dt) {
    ...
    Kalman->P[0][0] +=
        dt * (dt * Kalman->P[1][1] - Kalman->P[0][1] - Kalman->P[1][0] + Kalman->Q_angle);
...
void mpu6050_kalman(mpu6050_t *mpu6050) {
    // Kalman angle solve
    float dt = (float)(esp_timer_get_time() / 1000);
    ...
    if (fabs(mpu6050->kalman_angle_y) > 90) mpu6050->gx = -mpu6050->gx;
