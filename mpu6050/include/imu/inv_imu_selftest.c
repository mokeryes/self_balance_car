...
int inv_imu_load_selftest_data(inv_imu_device_t *s) {
    ...
    /* Set up OTP controller to reload factory-trimmed self-test response into
     * SRAM */
    ...
    /* Host should disable INT function first before kicking off OTP copy
     * operation */
...
static int configure_selftest_parameters(inv_imu_device_t *s,
    ...
    status |= inv_imu_read_reg(s, SELFTEST_MREG1, 1, &value);
    if (value & (SELFTEST_ACCEL_ST_EN_MASK | SELFTEST_GYRO_ST_EN_MASK)) return INV_ERROR_UNEXPECTED;
