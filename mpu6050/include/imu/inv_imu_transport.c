...
int inv_imu_init_transport(void *t) {
    ...
    if (tr == NULL) return INV_ERROR_BAD_ARG;
...
int inv_imu_read_reg(void *t, uint32_t reg, uint32_t len, uint8_t *buf) {
    ...
    if (t == NULL) return INV_ERROR_BAD_ARG;
...
int inv_imu_write_reg(void *t, uint32_t reg, uint32_t len, const uint8_t *buf) {
    ...
    for (uint32_t i = 0; i < len; i++) {
        ...
        if (cache_addr) *cache_addr = buf[i];

        if (!(reg & 0x10000)) rc |= write_mclk_reg(t, ((reg + i) & 0xFFFF), 1, &buf[i]);
    }

    if (reg & 0x10000) rc |= write_sreg(t, (uint8_t)reg, len, buf);
...
int inv_imu_switch_on_mclk(void *t) {
    ...
    if (tr->need_mclk_cnt == 0) {
        ...
        do {
            ...
            /* Timeout */
            if (inv_imu_get_time_us() - start > TIMEOUT_US) return INV_ERROR_TIMEOUT;

        } while (!(data & MCLK_RDY_MCLK_RDY_MASK));
    } else {
        /* Make sure it is already on */
        status |= inv_imu_read_reg(t, PWR_MGMT0, 1, &data);
        if (0 == (data &= PWR_MGMT0_IDLE_MASK)) status |= INV_ERROR;
    }
...
static uint8_t *get_register_cache_addr(void *t, const uint32_t reg) {
    ...
    if (tr == NULL) return (uint8_t *)0; /* error */
...
static int read_sreg(void *t, uint8_t reg, uint32_t len, uint8_t *buf) {
    ...
    if (serif == NULL) return INV_ERROR_BAD_ARG;

    if (len > serif->max_read) return INV_ERROR_SIZE;

    if (serif->read_reg(serif, reg, buf, len) != 0) return INV_ERROR_TRANSPORT;
...
static int write_sreg(void *t, uint8_t reg, uint32_t len, const uint8_t *buf) {
    ...
    if (len > serif->max_write) return INV_ERROR_SIZE;

    if (serif->write_reg(serif, reg, buf, len) != 0) return INV_ERROR_TRANSPORT;
...
static int read_mclk_reg(void *t, uint16_t regaddr, uint8_t rd_cnt, uint8_t *buf) {
    ...
    if (blk_sel) status |= write_sreg(t, (uint8_t)BLK_SEL_R & 0xff, 1, &blk_sel);
...
static int write_mclk_reg(void *t, uint16_t regaddr, uint8_t wr_cnt, const uint8_t *buf) {
    ...
    /* optimize by changing BLK_SEL only if not NULL */
    if (blk_sel) status |= write_sreg(t, (uint8_t)BLK_SEL_W, 1, &blk_sel);
